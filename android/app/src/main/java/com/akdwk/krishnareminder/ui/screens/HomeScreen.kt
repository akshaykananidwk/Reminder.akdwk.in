...
fun HomeScreen(
    ...
    // Re-checked every time the screen resumes. With a bare remember {} this
    // was read once and kept forever, so the warning stayed up after the user
    // had granted everything — and would not appear if a permission was later
    // revoked, which Samsung does on its own to sleeping apps.
    var permissionCheck by remember { mutableStateOf(0) }
    val permissionsOk = remember(permissionCheck) { PermissionUtils.allCriticalGranted(context) }
